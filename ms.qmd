...
```{r}
#| label: tbl-tasks
#| tbl-cap: "The LEVANTE core tasks, presented with their simplified labels as well as prior labels used in the literature, and citations for the task or source of inspiration."
...
The Corsi Block task is a widely-used measure of visuospatial short-term and working memory [@corsi1972]. In our version of the task modeled after the AMES tablet task version [@obradovic2025motivation], the child sees a grid of squares; during each trial, a subset of squares lights up one at a time in a specific sequence. The child is required to reproduce the sequence by touching the squares: in the first block, the respondent is asked to reproduce the sequence in the same order and in the second block, they are asked to reproduce it backwards. The task begins with short sequences (e.g., two items) and gradually increases in difficulty (up to seven items) until the child makes mistakes on two sequences of the same length. We used both 2x2 and 3x3 grids in our pilot testing, with relatively similar results. 

## Social Cognition

### Stories

Assessing social cognition across a wide range of ages is a challenge, and relatively few instruments show good reliability and validity across not just early childhood but older children as well [cf. @heise2025]. In addition, due to the lengthy nature of vignette-based assessments, it was challenging to identify an assessment that would fit in our allotted time. We chose to adapt and re-illustrate a storybook task developed by @sotomayor-enriquez2024 in which children hear stories with multiple social reasoning questions embedded, including questions about true and false beliefs, deception, and moral reasoning. Drawing on published data from this task, we selected items with good psychometric properties and strong potential for cross-cultural validity (e.g., stories commonly found across many cultures), and then supplemented these with additional questions assessing emotional reasoning. The result was a set of six stories, each with approximately five forced-choice (2 or 3 alternatives) social reasoning questions embedded within each story. We then developed two sets of alternative retest stories, in each case using the same structure as an existing story but substituting new details and characters.

<!-- move to Scoring section? -->
For each task we distinguish between trials and IRT items (see @tbl-trial-irt-counts). Trials refer to individual questions that could be presented to the child (design-level count; e.g., 60 trials in the non-adaptive Hearts & Flowers task). For psychometric analyses, we define a smaller set of IRT items, where repeated trials of the same type are treated as interchangeable replicates, and practice or feedback trials are excluded. The responses to these IRT items are what enters the models, and therefore have estimated discrimination and difficulty parameters.
...
#| fig-cap: "Marginal effect of age across all sites and within each site, representing an increase in standard deviations of ability per year of age."
#| fig-width: 9.5
#| fig-height: 3
...
To test whether fewer underlying factors would fit comparably well, we additionally compared a family of more parsimonious alternatives: a single-factor model, a two-factor model separating reasoning and language, and a three-factor model distinguishing reasoning, executive function, and language. These models all treated Math and Stories as indicators of their respective factors rather than as separate outcomes (necessary to construct nested alternatives with fewer factors). None of these alternatives fit better than the three-factor version (see @tbl-alt-models); because this comparison uses a different indicator specification than the model above, the fit indices in @tbl-alt-models differ slightly from those reported for @fig-semplot-core-tasks. We retain the model in @fig-semplot-core-tasks, treating Math and Stories as observed outcomes for the reasons outlined above; the factor-count comparison here provides no evidence that a different number of factors would fit better. Future work will pursue more complete models that examine if and how the constructs may be organized differently across different contexts and ages.
...
Given the very high inter-factor correlations in the three-factor comparison model above (@tbl-alt-models), we investigated whether a bifactor structure (i.e., a single general factor alongside domain-specific factors) might better characterize the data; see Appendix for the full analysis. This analysis extends the comparison specification from @tbl-alt-models (Math and Stories as indicators of their respective factors) rather than the model in @fig-semplot-core-tasks, since a bifactor decomposition requires every task to load on the general factor. A direct bifactor model produced an improper (Heywood-case) solution (@tbl-bifactor-fit-compare), which can have several possible causes (e.g., sampling variability or too few indicators per specific factor), but the pattern here converges with the model-free implied loadings (see Appendix: @tbl-analytic-g-loadings), consistent with specific factors carrying little variance beyond a dominant general factor. We therefore recovered general- and specific-factor loadings via a second-order model with a Schmid-Leiman transformation [@schmid1957development] (@tbl-bifactor-methods-comparison). Reasoning proved essentially indistinguishable from a general factor, while Executive Function and especially Language retained modestly more distinguishable variance. Explained common variance (ECV = .91) and omega-hierarchical ($\omega_{H}$ = .88) both exceeded conventional thresholds for treating the battery as substantially general-factor-dominated in practice [@reise2012rediscovery; @rodriguez2016evaluating] (@tbl-bifactor-indices), though our inter-factor correlations run higher than comparable estimates from a twin sample of Texas children (Reasoning–EF *r* = .84, EF–Language *r* = .76, Reasoning–Language *r* = .75; [@engelhardt2016]), particularly for Language. We note that comparative fit is not the basis for this characterization; bifactor models are known to exhibit elevated fitting propensity [@bonifay2017complexity; @bonifay2025reassessing], and our reading rests on the model-free implied loadings and variance-partitioning indices reported in Appendix A rather than on the fit comparison in Table A1.

This general-factor-dominated pattern does not appear to be an artifact of the pilot sample's site composition: repeating the correlated three-factor model on 200 equal-size-per-site subsamples produced highly consistent factor correlations (Reasoning–EF, Reasoning–Language, EF–Language all within $\pm.02$ of the full-sample estimates on average; @tbl-subsample-robustness, @fig-subsample-robustness in the appendix), and the same qualitative pattern held, albeit more noisily, within the two individual sites for which within-site model estimation was stable (@tbl-within-site-robustness).
...
# Bifactor and Schmid-Leiman analysis of the core task battery {.appendix #apx-bifactor}

Given the very high inter-factor correlations in the three-factor comparison model reported in the Construct Validity section (@tbl-alt-models), we investigated whether a bifactor structure -- a single general factor alongside domain-specific factors -- might better characterize the data. As in that comparison model, and unlike the main SEM in @fig-semplot-core-tasks, Math and Stories are treated here as indicators of the Reasoning and Language factors respectively, rather than as separate observed outcomes; this is the specification a bifactor decomposition requires, since every task must load on the general factor.

{{< include _09-bifactor.qmd >}}

# Site-imbalance robustness check {#apx-subsample}

Sample sizes are imbalanced across the pilot sites (see Data filtering), which raises the concern that the correlated three-factor model's high inter-factor correlations, and the bifactor results in Appendix, could be an artifact of pooling over unevenly-sized, unevenly-composed sites rather than a genuine feature of the tasks. We evaluate this directly with two complementary checks: repeatedly refitting the three-factor model on equal-size-per-site subsamples, and refitting it separately within each individual site (no cross-site pooling at all). The full-sample estimates fell at or within the 95% interval of these robustness analyses.
