...
task_items <- read_csv("04_papers/display/task_item-active test items.csv")
bank_items <- task_items |>
  rename(item_task = `task_short_name (from task)`) |>
  count(item_task, name = "bank_items")

exclusions <- read_csv("04_papers/display/exclusion-Grid view.csv") |> select(item)
coef_df <- task_coefs |>
...
ages <- coretask_scores |> summarise(age = min(age), .by = c(site_label, user_id))
...
```{r}
#| label: tbl-task-rxx
#| tbl-cap: "Marginal reliabilities for each task and site for tasks with scalar invariance"
...
#| tbl-cap: "Marginal reliabilities for each task and site for tasks with configural inariance."
